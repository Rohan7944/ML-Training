...
tokens = enc.encode("Hello, I am a language model") # 7 tokens (tokenizer vercel app)
tokens = torch.tensor(tokens, dtype=torch.long) # (7,)
tokens = tokens.unsqueeze(0).repeat(num_return_sequences, 1) # (num_return_sequences, 7)
x = tokens.to(device) # (num_return_sequences, 7)

# generate right now x is (B, T) where B = num_return_sequences and T = 7
# set seed to 42
torch.manual_seed(42)
# torch.cuda.manual_seed_all(42) # uncomment this to run on GPU, if available
while x.size(1) < max_length:
    ...
    with torch.no_grad():
        ...
        # topk_probs here becomes (B, 50) and topk_indices becomes (B, 50)
