...
with open("input.txt", "r") as f:
    text = f.read()
text = text[:1000] # only take the first 1000 characters for speed
tokens = enc.encode(text) # encode the text to tokens
B, T = 4, 32 
buf = torch.tensor(tokens[:B*T + 1]) # +1 because we need ground truth for the last token
buf = buf.to(device)
...
# print(logits.shape) # torch.Size([4, 32, 50257])
# print(loss)  # tensor(11.1197, grad_fn=<NllLossBackward0>) expected -ln(1/50257) = 10.82
